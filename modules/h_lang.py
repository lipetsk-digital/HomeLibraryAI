...
from modules.imports_tg import asyncpg, Bot, Router, F, Chat, User, FSMContext, _, Command, CallbackQuery, InlineKeyboardBuilder, Locale, env, engt
import modules.h_start as h_start # For handling start command

# Show menu for language selection
@engt.base_router.message(Command("settings"))
async def SelectLanguage(state: FSMContext, pool: asyncpg.Pool, bot: Bot, event_chat: Chat, event_from_user: User) -> None:
    await engt.RemoveInlineKeyboards(None, state, bot, event_chat.id)
    builder = InlineKeyboardBuilder()
    available_languages = sorted(engt.i18n.available_locales)
...
@engt.base_router.callback_query(env.Language.filter())
async def lang_callback(callback: CallbackQuery, callback_data: env.Language, state: FSMContext, pool: asyncpg.Pool, bot: Bot, event_chat: Chat, event_from_user: User) -> None:
    await engt.RemoveInlineKeyboards(callback, state, bot, event_chat.id)
    # Change locale
    await engt.FSMi18n.set_locale(state, callback_data.lang)
